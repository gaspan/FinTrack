...
export async function reconcileWalletBalances(db: SQLiteDatabase): Promise<void> {
  const wallets = await db.getAllAsync<{ id: number; balance: number; initial_balance: number; book_id: number }>(
    'SELECT id, balance, initial_balance, book_id FROM wallets'
  );
  ...
  for (const w of wallets) {
    const { total } = await db.getFirstAsync<{ total: number }>(
      `SELECT COALESCE(SUM(CASE WHEN type = 'income' THEN amount ELSE -amount END), 0) as total
       FROM transactions WHERE book_id = ? AND wallet_id = ?`,
      [w.book_id, w.id]
    ) ?? { total: 0 };
