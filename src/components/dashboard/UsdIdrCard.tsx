...
import { FX_RANGES, getUsdIdrSeries, type FxRange, type FxSeries } from '@/features/fx/usdIdr';

const CHART_H = 150;

export const UsdIdrCard: React.FC = () => {
  ...
  const values = useMemo(() => series?.points.map((p) => p.rate) ?? [], [series]);
  ...
  return (
    <>
      <SegmentTabs
        options={FX_RANGES.map((r) => ({ key: r.key, label: r.label }))}
        value={range}
        onChange={(k) => {
          setLoading(true);
          setFailed(false);
          setRange(k as FxRange);
        }}
        labelPrefix="Rentang"
      />
      ...
      {loading ? (
        <View style={styles.loadingWrap}>
          <Skeleton height={26} width="45%" style={{ marginBottom: 6 }} />
        ...
        <View style={styles.errorWrap}>
          ...
          <Text style={styles.errorTitle}>Kurs tak tersedia</Text>
        ...
        <>
          <View style={styles.priceRow}>
            <View style={styles.priceWrap}>
              <Text style={styles.priceLabel}>1 USD ke Rupiah</Text>
              <Text style={styles.price} numberOfLines={1} adjustsFontSizeToFit>
                {formatRupiah(Math.round(series.last))}
              </Text>
            </View>
          ...
          <MarketAreaChart
            ...
            gradientId="usdIdrFill"
            formatValue={(v) => formatRupiahShort(Math.round(v))}
          />

          <Text style={styles.source}>Sumber: Frankfurter (data referensi ECB)</Text>
        </>
      )}
    </>
  );
