...
export const AssetsList: React.FC<AssetsListProps> = ({ items, onDelete }) => {
  const { theme } = useTheme();
  const styles = makeStyles(theme);

  const handleDelete = (item: Asset) => {
    Alert.alert('Hapus Aset', `Hapus "${item.name}"?`, [
  ...
  if (items.length === 0) {
    return <Text style={styles.empty}>Belum ada aset</Text>;
  }
    ...
    <View style={styles.list}>
      {items.map((item) => (
        ...
        >
          ...
          <Text style={styles.value}>+{formatRupiah(item.current_value)}</Text>
        </TouchableOpacity>
      ))}
    </View>
  );
...
const makeStyles = (theme: Theme) => StyleSheet.create({
  ...
  value: { fontWeight: '700', fontSize: 14, color: '#10B981' },
});